...
const AchievementsSection = () => {
  ...
  useEffect(() => {
    ...
    const elements = sectionRef.current?.querySelectorAll(".reveal-scale, .reveal-fade");
  ...
  return (
    <section id="achievements" ref={sectionRef} className="py-32 relative overflow-hidden ambient-glow">
      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          {achievements.map((a, index) => (
            <TiltCard key={index} className="reveal-scale glass-card-hover p-8 group">
              <div className="flex flex-col md:flex-row gap-6">
                <div className={`w-14 h-14 rounded-2xl ${a.bgColor} flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-500`}>
                ...
                <div className="flex-1">
                  ...
                  {a.links && (
                    <div className="flex flex-wrap gap-3">
                      {a.links.map((link, li) => (
                        <a key={li} href={link.url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:text-accent transition-colors duration-300">
                          {link.label} →
                        </a>
                      ))}
                    </div>
