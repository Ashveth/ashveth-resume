...
const ProjectsSection = () => {
  ...
  useEffect(() => {
    ...
    const elements = sectionRef.current?.querySelectorAll(".reveal-rotate, .reveal-fade");
  ...
  return (
    <section id="projects" ref={sectionRef} className="py-32 relative overflow-hidden ambient-glow">
          ...
          {projects.map((project, index) => (
            <TiltCard key={index} delay={index * 0.05}>
              <a
                ...
                className="reveal-rotate block glass-card-hover p-8 group cursor-pointer h-full"
              >
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${project.gradient} flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 ease-out`}>
