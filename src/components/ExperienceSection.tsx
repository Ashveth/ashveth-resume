      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-6xl">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          {experiences.map((exp, index) => (
            <div
              key={index}
              className="reveal-left glass-card p-6 md:p-8 hover:scale-[1.01] transition-all duration-300 group"
            >
              <div className="flex flex-col sm:flex-row gap-6">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-3">
                    <h3 className="text-xl md:text-2xl font-display font-semibold">{exp.title}</h3>
                    <span className="text-sm text-primary font-medium whitespace-nowrap">{exp.period}</span>
                  </div>
                  <h4 className="text-base md:text-lg text-foreground/80 font-medium mb-3">{exp.company}</h4>
                  <p className="text-foreground/70 leading-relaxed text-sm md:text-base">{exp.description}</p>
                </div>
              </div>
            </div>
          ))}
